...
const CountryStat=props=>{
    ...
    return(
        ...
        <div className='country_stat_container' style={{background:props.isDark?'#2e315d':'#fff'}}>
            <div>
                <div className='flag_name_wrap'>
                    <img src={flagUrl} alt={props.name} className='flag_img'/>
                    <div className='country_name' style={{color:props.isDark?'#ccccea':'#30313A'}}>{props.name}</div>
                </div>
                <div>
                    <span className="stat" 
